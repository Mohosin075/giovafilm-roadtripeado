...
import { downloadImageToUploads } from '../utils/remoteImage'
...
const nameScore = (left: string, right: string): number => {
  ...
  if (isSubstring && (shorter.includes(' ') || shorter.length >= 6)) return 0.85
...
const rankCandidates = (candidates: Candidate[]): Candidate[] => {
  const acceptable = candidates.filter(candidate => {
    if (candidate.distance <= SAME_PIN_KM) return true
    if (
    ...
    return (
      candidate.distance <= TEXT_MATCH_KM && candidate.score >= STRONG_NAME_SCORE
    )
  })
...
const searchGoogle = async (
  ...
  if (coordinates) {
    ...
    candidates.push(
      ...toCandidates(nearby.data?.results, coordinates, placeName, 'nearby'),
    )
  }
  ...
  candidates.push(
    ...toCandidates(text.data?.results, coordinates, placeName, 'text'),
  )

  return candidates
}

const fetchPhotoReferences = async (placeId: string): Promise<string[]> => {
...
const run = async () => {
  ...
  for (const place of empty) {
    ...
    const candidates = await searchGoogle(
      place.name,
      place.country || 'Puerto Rico',
      coordinates,
    )
    ...
    if (!match) {
      console.log(
        `⏭️  ${place.name} → "${ranked[0].name}" (${ranked[0].distance.toFixed(1)} km) — no photos`,
      )
      noPhoto++
      continue
    }
    ...
    console.log(
      `✅ ${place.name} → "${match.name}" (${match.distance.toFixed(1)} km, score ${match.score.toFixed(2)}, ${references.length} photo)`,
    )
    ...
    for (const reference of references) {
      const localPath = await downloadImageToUploads(
        `https://maps.googleapis.com/maps/api/place/photo?maxwidth=1000&photo_reference=${reference}&key=${GOOGLE_API_KEY}`,
      )
      if (localPath) media.push(localPath)
    }
