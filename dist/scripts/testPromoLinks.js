"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const mongoose_1 = __importDefault(require("mongoose"));
const dotenv_1 = __importDefault(require("dotenv"));
const path_1 = __importDefault(require("path"));
const user_model_1 = require("../modules/user/user.model");
const map_model_1 = require("../modules/map/map.model");
const promo_model_1 = require("../modules/promo/promo.model");
const promo_service_1 = require("../modules/promo/promo.service");
dotenv_1.default.config({ path: path_1.default.join(process.cwd(), '.env') });
const DATABASE_URL = process.env.DATABASE_URL;
async function testPromoLinks() {
    var _a, _b;
    if (!DATABASE_URL) {
        console.error('DATABASE_URL is not defined in .env file');
        return;
    }
    console.log('Connecting to database:', DATABASE_URL);
    await mongoose_1.default.connect(DATABASE_URL);
    try {
        // 1. Get a map to use for testing
        let map = await map_model_1.Map.findOne();
        if (!map) {
            console.log('Creating a test map...');
            map = await map_model_1.Map.create({
                name: 'Test Map Puerto Rico',
                description: 'Test Description',
                isPaid: true,
                price: 15.0,
                status: 'Active',
            });
        }
        console.log(`Using Map: ${map.name} (${map._id.toString()})`);
        // 2. Get a user to use for testing
        let user = await user_model_1.User.findOne();
        if (!user) {
            console.log('Creating a test user...');
            user = await user_model_1.User.create({
                name: 'Test User',
                email: 'testpromo@example.com',
                role: 'user',
                status: 'active',
                purchasedMaps: [],
            });
        }
        console.log(`Using User: ${user.name} (${user.email})`);
        // Clean up any old test promo codes
        await promo_model_1.PromoLink.deleteMany({ label: { $regex: /Test Promo/ } });
        // 3. Test Bulk Generation
        console.log('\n--- Test 1: Bulk Generation ---');
        const generated = await promo_service_1.PromoServices.bulkGeneratePromoLinks({
            mapId: map._id.toString(),
            price: 0, // Free link
            promoType: 'influencer',
            label: 'Test Promo Free',
            emails: [user.email],
            expiresAt: new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString(), // 1 day expiry
        });
        if (generated.length === 1) {
            console.log(`✅ Bulk generated 1 free code successfully: ${generated[0].code}`);
        }
        else {
            throw new Error('Failed to bulk generate free code');
        }
        const generatedPaid = await promo_service_1.PromoServices.bulkGeneratePromoLinks({
            mapId: map._id.toString(),
            price: 5.0, // Paid link
            promoType: 'upgrade',
            label: 'Test Promo Paid',
            emails: [user.email],
        });
        if (generatedPaid.length === 1) {
            console.log(`✅ Bulk generated 1 paid code successfully: ${generatedPaid[0].code}`);
        }
        else {
            throw new Error('Failed to bulk generate paid code');
        }
        // 4. Test Verification
        console.log('\n--- Test 2: Verify Code ---');
        const verifiedFree = await promo_service_1.PromoServices.verifyPromoCode(generated[0].code);
        console.log(`✅ Verified Free code: ${verifiedFree.code}, Price: ${verifiedFree.price}, Type: ${verifiedFree.promoType}, Map: ${verifiedFree.mapName}`);
        const verifiedPaid = await promo_service_1.PromoServices.verifyPromoCode(generatedPaid[0].code);
        console.log(`✅ Verified Paid code: ${verifiedPaid.code}, Price: ${verifiedPaid.price}, Type: ${verifiedPaid.promoType}, Map: ${verifiedPaid.mapName}`);
        // Remove map from user's purchasedMaps first to test claiming
        await user_model_1.User.findByIdAndUpdate(user._id, {
            $pull: { purchasedMaps: map._id },
        });
        // 5. Test Claim Free Promo Code
        console.log('\n--- Test 3: Claim Free Promo ---');
        const claimRes = await promo_service_1.PromoServices.claimFreePromo(user._id.toString(), generated[0].code);
        console.log('Claim Response:', claimRes);
        // Verify map is in user's purchasedMaps
        const updatedUser = await user_model_1.User.findById(user._id);
        const isUnlocked = (_a = updatedUser === null || updatedUser === void 0 ? void 0 : updatedUser.purchasedMaps) === null || _a === void 0 ? void 0 : _a.some((id) => id.toString() === map._id.toString());
        if (isUnlocked) {
            console.log('✅ Map unlocked successfully in user account');
        }
        else {
            throw new Error('Map was not found in user account after claim');
        }
        // Verify link status
        const claimedLink = await promo_model_1.PromoLink.findOne({ code: generated[0].code });
        if ((claimedLink === null || claimedLink === void 0 ? void 0 : claimedLink.isUsed) &&
            ((_b = claimedLink === null || claimedLink === void 0 ? void 0 : claimedLink.usedBy) === null || _b === void 0 ? void 0 : _b.toString()) === user._id.toString()) {
            console.log('✅ Promo link status updated to used with correct userId');
        }
        else {
            throw new Error('Promo link record was not correctly updated');
        }
        // 6. Test Double Claim Prevention
        console.log('\n--- Test 4: Prevent Double Claiming ---');
        try {
            await promo_service_1.PromoServices.claimFreePromo(user._id.toString(), generated[0].code);
            throw new Error('Double claim should have thrown an error');
        }
        catch (err) {
            console.log(`✅ Double claiming correctly blocked: ${err.message}`);
        }
        // 7. Simulate Webhook Checkout Processing for Paid Promo
        console.log('\n--- Test 5: Simulate Webhook / Paid Upgrade ---');
        // We clean up map access again
        await user_model_1.User.findByIdAndUpdate(user._id, {
            $pull: { purchasedMaps: map._id },
        });
        // Simulate the PromoLink update when payment is received (the logic implemented in webhooks)
        const promoCode = generatedPaid[0].code;
        const promoLink = await promo_model_1.PromoLink.findOneAndUpdate({ code: promoCode, isUsed: false }, {
            isUsed: true,
            usedBy: user._id,
            usedAt: new Date(),
        }, { new: true });
        if (promoLink && promoLink.isUsed) {
            await user_model_1.User.findByIdAndUpdate(user._id, {
                $addToSet: { purchasedMaps: promoLink.mapId },
            });
            console.log('✅ Simulated Stripe payment processing successfully unlocked the map and marked the promo code as used');
        }
        else {
            throw new Error('Failed to simulate Stripe payment processing');
        }
        // Clean up
        await promo_model_1.PromoLink.deleteMany({ label: { $regex: /Test Promo/ } });
        console.log('\n✅ All tests passed successfully!');
    }
    catch (error) {
        console.error('❌ Test failed:', error);
    }
    finally {
        await mongoose_1.default.disconnect();
        console.log('Disconnected from database');
    }
}
testPromoLinks();
