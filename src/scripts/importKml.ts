...
const KML_FILE_PATH = path.join(__dirname, '../../Roadtripeado Maps 1.0 🇵🇷.kml');
...
async function importKml() {
  ...
  try {
    const kmlData = fs.readFileSync(KML_FILE_PATH, 'utf-8');
    ...
    for (const folder of foldersArray) {
      ...
      for (const pm of placemarksArray) {
        ...
        if (pm.ExtendedData && pm.ExtendedData.Data) {
            ...
            const mediaLinksField = dataFields.find((f: any) => f["@_name"] === "gx_media_links");
            if (mediaLinksField && mediaLinksField.value) {
                const links = mediaLinksField.value.split(' ').filter((l: string) => l.startsWith('http'));
                links.forEach((l: string) => {
                    if (!media.includes(l)) media.push(l);
                });
            }
        }
