...
import { downloadImageToUploads, isRemoteManagedImage } from '../utils/remoteImage'
 ...
 */
const localizeList = async (
  list: unknown,
  label: string,
): Promise<{ next: string[]; changed: number; failed: number }> => {
  ...
  for (const url of urls) {
    if (!isRemoteManagedImage(url)) {
      next.push(url)
      continue
    }

    const localPath = await downloadImageToUploads(url)
    if (localPath) {
      next.push(localPath)
      changed++
...
const run = async () => {
  ...
  const placesToFix = places.filter(
    (place: any) =>
      (place.media || []).some(isRemoteManagedImage) ||
      (place.menuImages || []).some(isRemoteManagedImage),
  )

  console.log(`📍 Places with remote images: ${placesToFix.length}`)

  for (const place of placesToFix as any[]) {
    console.log(`\n🔧 Place: ${place.name}`)
  ...
  const businessesToFix = businesses.filter((business: any) =>
    (business.media?.photos || []).some(isRemoteManagedImage),
  )

  console.log(`\n🏢 Businesses with remote images: ${businessesToFix.length}`)

  for (const business of businessesToFix as any[]) {
    console.log(`\n🔧 Business: ${business.name}`)
    const photos = await localizeList(business.media?.photos, 'photo')
